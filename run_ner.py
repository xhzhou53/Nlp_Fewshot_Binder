...
def main():
    ...
    # ================= 插入开始 =================
    # 这里的 raw_datasets 是 conll 格式 (tokens + ner indices)
    # Binder 需要 char-level 格式 (text + char offsets)
    # 我们在这里做一个自动转换
    logger.info("正在将 CoNLL 格式自动转换为 Binder 需要的 Character-Offset 格式...")

    # 修改函数定义，增加 idx 参数
    def adapt_conll_to_binder(example, idx):
        ...
        for i, token in enumerate(tokens):
            ...
            word_start_chars.append(start)
            word_end_chars.append(end)
            token_map[i] = (start, end)
        ...
        for entity in ner_raw:
            if isinstance(entity, dict):
                ...
                t_start, t_end, label = entity
            
            # 只有当实体的 start 和 end 都在我们的 token 映射里时才添加
            if t_start in token_map and t_end in token_map:
                char_start = token_map[t_start][0]
                char_end = token_map[t_end][1]
                
                entity_types.append(label)
                entity_start_chars.append(char_start)
                entity_end_chars.append(char_end)
        
        return {
            "id": str(idx),  # <--- 【关键修改】这里变成了动态 ID！
            "text": text,
    ...
    if "train" in raw_datasets:
        raw_datasets = raw_datasets.map(adapt_conll_to_binder, with_indices=True,load_from_cache_file=False)
